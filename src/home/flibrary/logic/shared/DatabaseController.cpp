...
std::unique_ptr<DB::IDatabase> CreateDatabaseImpl(const std::string & databaseName)
{
	...
	transaction->CreateCommand("CREATE TABLE IF NOT EXISTS Searches_User(SearchID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, Title VARCHAR(150) NOT NULL UNIQUE COLLATE NOCASE)")->Execute();
	transaction->Commit();
