...
void registerMetaType(MetaObjects & metaObjects, QString id, MetaPair metaPair)
{
	...
	QObject::connect(metaPair.first, &QObject::destroyed, [&metaObjects, id = std::move(id), metaPair = std::move(metaPair)] ()
	{
...
void ObjectsConnector::registerEmitter(QString ID, QObject * sender, const QString & signal_, bool queued /* = false*/)
{
	...
	for (const auto & [receiver, slot] : g_slots[ID])
	{
		...
		[[maybe_unused]] const auto result = connect(sender, signal.data(), receiver, slot.data()
			, queued ? static_cast<Qt::ConnectionType>(Qt::QueuedConnection | Qt::UniqueConnection) : Qt::UniqueConnection);
