...
concept BinaryLogicalPredicate = requires(const std::remove_reference_t<Predicate>& predicate,
                                          const std::remove_reference_t<Value>& lhs,
...
InputIterator FindPairIteratorByFirst(InputIterator begin,
                                      InputIterator end,
                                      const typename Value::first_type& key,
                                      KeyEqual comparer = KeyEqual {})
{
	return std::find_if(begin, end, [&](const auto it) { return comparer(key, it.first); });
}
...
FindPairIteratorByFirst(const Container& container, const typename Value::first_type& key, KeyEqual comparer = KeyEqual {})
{
...
template <typename InputIterator,
          typename Value = typename std::iterator_traits<InputIterator>::value_type,
          BinaryLogicalPredicate<typename Value::first_type> KeyEqual = std::equal_to<typename Value::first_type>>
const typename Value::second_type& FindSecond(InputIterator begin,
                                              InputIterator end,
                                              const typename Value::first_type& key,
                                              const typename Value::second_type& defaultValue,
                                              KeyEqual comparer = KeyEqual {})
{
	const auto it = FindPairIteratorByFirst(begin, end, key, comparer);
...
template <typename Container,
          typename Value = typename Container::value_type,
          BinaryLogicalPredicate<typename Value::first_type> KeyEqual = std::equal_to<typename Value::first_type>>
const typename Value::second_type& FindSecond(const Container& container,
                                              const typename Value::first_type& key,
                                              const typename Value::second_type& defaultValue,
                                              KeyEqual comparer = KeyEqual {})
{
...
template <typename Value,
          size_t ArraySize,
          BinaryLogicalPredicate<typename Value::first_type> KeyEqual = std::equal_to<typename Value::first_type>>
const typename Value::second_type& FindSecond(Value (&array)[ArraySize],
                                              const typename Value::first_type& key,
                                              const typename Value::second_type& defaultValue,
                                              KeyEqual comparer = KeyEqual {})
{
...
InputIterator FindPairIteratorBySecond(InputIterator begin,
                                       InputIterator end,
                                       const typename Value::second_type& key,
                                       KeyEqual comparer = KeyEqual {})
{
	return std::find_if(begin, end, [&](const auto& it) { return comparer(key, it.second); });
}
...
template <typename InputIterator,
          typename Value = typename std::iterator_traits<InputIterator>::value_type,
          BinaryLogicalPredicate<typename Value::second_type> KeyEqual = std::equal_to<typename Value::second_type>>
const typename Value::first_type& FindFirst(InputIterator begin,
                                            InputIterator end,
                                            const typename Value::second_type& key,
                                            const typename Value::first_type& defaultValue,
                                            KeyEqual comparer = KeyEqual {})
{
	const auto it = FindPairIteratorBySecond(begin, end, key, comparer);
...
template <typename Container,
          typename Value = typename Container::value_type,
          BinaryLogicalPredicate<typename Value::second_type> KeyEqual = std::equal_to<typename Value::second_type>>
const typename Value::first_type& FindFirst(const Container& container,
                                            const typename Value::second_type& key,
                                            const typename Value::first_type& defaultValue,
                                            KeyEqual comparer = KeyEqual {})
{
...
template <typename Value,
          size_t ArraySize,
          BinaryLogicalPredicate<typename Value::second_type> KeyEqual = std::equal_to<typename Value::second_type>>
const typename Value::first_type& FindFirst(Value (&array)[ArraySize],
                                            const typename Value::second_type& key,
                                            const typename Value::first_type& defaultValue,
                                            KeyEqual comparer = KeyEqual {})
{
...
struct PszComparer
{
	bool operator()(std::string_view lhs, std::string_view rhs) const { return compare(lhs, rhs); }

	bool operator()(std::span<const unsigned char> lhs, std::string_view rhs) const { return compare(lhs, rhs); }

	bool operator()(std::string_view lhs, std::span<const unsigned char> rhs) const { return compare(lhs, rhs); }

private:
	template <typename L, typename R>
	bool compare(L lhs, R rhs) const
	{
		const auto [lit, rit] = std::ranges::mismatch(lhs,
		                                              rhs,
		                                              [](const unsigned char lhs, const char rhs)
