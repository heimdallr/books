...
// flag might be manually specified in the bitdefines.hpp header (included by formatdetect.hpp).
	...
	#include "impl/7z-sdk/7z/CPP/7zip/IStream.h"
 ...
 */
auto constexpr str_hash(const bit7z::tchar* input) -> uint64_t
...
auto detect_format_from_signature(IStream* stream) -> const BitInFormat&
{
	...
	if (isIso || fileSignature == kBeaSignature)
	{
		...
		for (auto descriptorIndex = 1; descriptorIndex < kMaxVolumeDescriptors; ++descriptorIndex)
		{
			...
			if (fileSignature == kUdfSignature)
			{ // The file is ISO+UDF or just UDF
				stream->Seek(to_large_integer(0), 0, nullptr);
				return BitFormat::Udf;
			}
		}
...
{
	tstring ext = inFile.extension().string().substr(1);
	if (ext.empty())
	{
	...
	if (format != nullptr)
	{ //extension found in the map
		return *format;
	}

	// Detecting multi-volume archives extensions
	if ((ext[0] == BIT7Z_STRING('r') || ext[0] == BIT7Z_STRING('z')) && (ext.size() == 3 && is_digit(ext[1]) && is_digit(ext[2])))
	{
