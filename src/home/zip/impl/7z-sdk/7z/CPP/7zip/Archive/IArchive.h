...
#define Z7_IFACE_CONSTR_ARCHIVE_SUB(i, base, n) Z7_DECL_IFACE_7ZIP_SUB(i, base, 6, n) { Z7_IFACE_COM7_PURE(i) };

#define Z7_IFACE_CONSTR_ARCHIVE(i, n) Z7_IFACE_CONSTR_ARCHIVE_SUB(i, IUnknown, n)
...
#define TIME_PREC_TO_ARC_FLAGS_TIME_DEFAULT(v) ((UInt32)(v) << NArcInfoTimeFlags::kTime_Prec_Default_bit_index)
...
#define Z7_IFACEM_IArchiveOpenCallback(x) x(SetTotal(const UInt64* files, const UInt64* bytes)) x(SetCompleted(const UInt64* files, const UInt64* bytes))
...
#define Z7_IFACEM_IArchiveExtractCallback(x) x(GetStream(UInt32 index, ISequentialOutStream** outStream, Int32 askExtractMode)) x(PrepareOperation(Int32 askExtractMode)) x(SetOperationResult(Int32 opRes))
...
*/
#define Z7_IFACEM_IArchiveExtractCallbackMessage2(x) x(ReportExtractResult(UInt32 indexType, UInt32 index, Int32 opRes))
Z7_IFACE_CONSTR_ARCHIVE(IArchiveExtractCallbackMessage2, 0x22)

#define Z7_IFACEM_IArchiveOpenVolumeCallback(x) x(GetProperty(PROPID propID, PROPVARIANT* value)) x(GetStream(const wchar_t* name, IInStream** inStream))
Z7_IFACE_CONSTR_ARCHIVE(IArchiveOpenVolumeCallback, 0x30)

#define Z7_IFACEM_IInArchiveGetStream(x) x(GetStream(UInt32 index, ISequentialInStream** stream))
Z7_IFACE_CONSTR_ARCHIVE(IInArchiveGetStream, 0x40)

#define Z7_IFACEM_IArchiveOpenSetSubArchiveName(x) x(SetSubArchiveName(const wchar_t* name))
...
#define Z7_IFACEM_IInArchive(x)                                                                                                                                                                \
	x(Open(IInStream* stream, const UInt64* maxCheckStartPosition, IArchiveOpenCallback* openCallback)) x(Close()) x(GetNumberOfItems(UInt32* numItems))                                       \
		x(GetProperty(UInt32 index, PROPID propID, PROPVARIANT* value)) x(Extract(const UInt32* indices, UInt32 numItems, Int32 testMode, IArchiveExtractCallback* extractCallback))           \
			x(GetArchiveProperty(PROPID propID, PROPVARIANT* value)) x(GetNumberOfProperties(UInt32* numProps)) x(GetPropertyInfo(UInt32 index, BSTR* name, PROPID* propID, VARTYPE* varType)) \
				x(GetNumberOfArchiveProperties(UInt32* numProps)) x(GetArchivePropertyInfo(UInt32 index, BSTR* name, PROPID* propID, VARTYPE* varType))
...
#define Z7_IFACEM_IArchiveGetRawProps(x)                                                                                                                                                                 \
	x(GetParent(UInt32 index, UInt32* parent, UInt32* parentType)) x(GetRawProp(UInt32 index, PROPID propID, const void** data, UInt32* dataSize, UInt32* propType)) x(GetNumRawProps(UInt32* numProps)) \
		x(GetRawPropInfo(UInt32 index, BSTR* name, PROPID* propID))

Z7_IFACE_CONSTR_ARCHIVE(IArchiveGetRawProps, 0x70)

#define Z7_IFACEM_IArchiveGetRootProps(x) x(GetRootProp(PROPID propID, PROPVARIANT* value)) x(GetRootRawProp(PROPID propID, const void** data, UInt32* dataSize, UInt32* propType))

Z7_IFACE_CONSTR_ARCHIVE(IArchiveGetRootProps, 0x71)

#define Z7_IFACEM_IArchiveOpenSeq(x) x(OpenSeq(ISequentialInStream* stream))
...
#define Z7_IFACEM_IArchiveUpdateCallback(x)                                                                                                                     \
	x(GetUpdateItemInfo(UInt32 index, Int32* newData, Int32* newProps, UInt32* indexInArchive)) x(GetProperty(UInt32 index, PROPID propID, PROPVARIANT* value)) \
		x(GetStream(UInt32 index, ISequentialInStream** inStream)) x(SetOperationResult(Int32 operationResult))

Z7_IFACE_CONSTR_ARCHIVE_SUB(IArchiveUpdateCallback, IProgress, 0x80)

// INTERFACE_IArchiveUpdateCallback(x)
#define Z7_IFACEM_IArchiveUpdateCallback2(x) x(GetVolumeSize(UInt32 index, UInt64* size)) x(GetVolumeStream(UInt32 index, ISequentialOutStream** volumeStream))
...
#define Z7_IFACEM_IArchiveUpdateCallbackFile(x) x(GetStream2(UInt32 index, ISequentialInStream** inStream, UInt32 notifyOp)) x(ReportOperation(UInt32 indexType, UInt32 index, UInt32 notifyOp))

Z7_IFACE_CONSTR_ARCHIVE(IArchiveUpdateCallbackFile, 0x83)

#define Z7_IFACEM_IArchiveGetDiskProperty(x) x(GetDiskProperty(UInt32 index, PROPID propID, PROPVARIANT* value))
...
#define Z7_IFACEM_IOutArchive(x) x(UpdateItems(ISequentialOutStream* outStream, UInt32 numItems, IArchiveUpdateCallback* updateCallback)) x(GetFileTimeType(UInt32* type))
...
#define Z7_IFACEM_ISetProperties(x) x(SetProperties(const wchar_t* const* names, const PROPVARIANT* values, UInt32 numProps))

Z7_IFACE_CONSTR_ARCHIVE(ISetProperties, 0x03)

#define Z7_IFACEM_IArchiveKeepModeForNextOpen(x) x(KeepModeForNextOpen())
...
#define Z7_IFACEM_IArchiveAllowTail(x) x(AllowTail(Int32 allowTail))
...
*/
#define Z7_IFACEM_IArchiveRequestMemoryUseCallback(x) x(RequestMemoryUse(UInt32 flags, UInt32 indexType, UInt32 index, const wchar_t* path, UInt64 requiredSize, UInt64* allowedSize, UInt32* answerFlags))
...
#define IMP_IInArchive_Props_WITH_NAME IMP_IInArchive_GetProp_WITH_NAME(GetNumberOfProperties, GetPropertyInfo, kProps)

#define IMP_IInArchive_ArcProps IMP_IInArchive_GetProp_NO_NAME(GetNumberOfArchiveProperties, GetArchivePropertyInfo, kArcProps)

#define IMP_IInArchive_ArcProps_WITH_NAME IMP_IInArchive_GetProp_WITH_NAME(GetNumberOfArchiveProperties, GetArchivePropertyInfo, kArcProps)
...
#define IMP_IInArchive_ArcProps_NO                                                                          \
	IMP_IInArchive_ArcProps_NO_Table Z7_COM7F_IMF(CHandler::GetArchiveProperty(PROPID, PROPVARIANT* value)) \
	{                                                                                                       \
		value->vt = VT_EMPTY;                                                                               \
		return S_OK;                                                                                        \
	}

#define Z7_class_CHandler_final Z7_class_final(CHandler)

#define Z7_CLASS_IMP_CHandler_IInArchive_0 Z7_CLASS_IMP_COM_1(CHandler, IInArchive)
#define Z7_CLASS_IMP_CHandler_IInArchive_1(i1) Z7_CLASS_IMP_COM_2(CHandler, IInArchive, i1)
#define Z7_CLASS_IMP_CHandler_IInArchive_2(i1, i2) Z7_CLASS_IMP_COM_3(CHandler, IInArchive, i1, i2)
#define Z7_CLASS_IMP_CHandler_IInArchive_3(i1, i2, i3) Z7_CLASS_IMP_COM_4(CHandler, IInArchive, i1, i2, i3)
#define Z7_CLASS_IMP_CHandler_IInArchive_4(i1, i2, i3, i4) Z7_CLASS_IMP_COM_5(CHandler, IInArchive, i1, i2, i3, i4)
#define Z7_CLASS_IMP_CHandler_IInArchive_5(i1, i2, i3, i4, i5) Z7_CLASS_IMP_COM_6(CHandler, IInArchive, i1, i2, i3, i4, i5)
...
#define GET_FileTimeType_NotDefined_for_GetFileTimeType NFileTimeType::kWindows
